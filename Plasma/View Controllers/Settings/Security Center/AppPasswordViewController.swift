...
class AppPasswordViewController: UIViewController, UITextFieldDelegate, UINavigationControllerDelegate {
    ...
    private func hash(_ text: String) -> Data? {
        return Crypto.sha256hash(text).hex
    }
    ...
    private func passwordSet() {
        DispatchQueue.main.async { [weak self] in
            let alert = UIAlertController(title: "Password set 🔐", message: "Forgetting this password will mean the app becomes completely bricked!", preferredStyle: UIAlertController.Style.alert)
